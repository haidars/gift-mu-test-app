...
function get_cookies(request, url) {
  const initial_cookies = parse_1(request.headers.get("cookie") ?? "");
  const new_cookies = [];
  const cookies = {
    get(name, opts) {
      ...
      while (i--) {
        ...
        if (cookie.name === name && domain_matches(url.hostname, cookie.options.domain) && path_matches(url.pathname, cookie.options.path)) {
          return cookie.value;
        }
      }
      return name in initial_cookies ? decode2(initial_cookies[name]) : void 0;
    },
...
function create_fetch({ event, options, state, route, prerender_default, resolve_opts }) {
  ...
  function get_cookie_header(url, header) {
    ...
    for (const cookie of set_cookies) {
      if (!domain_matches(url.hostname, cookie.domain))
        continue;
      if (!path_matches(url.pathname, cookie.path))
        continue;
      ...
      if (route) {
        ...
        for (const new_cookie of new_cookies) {
          response.headers.append(
            "set-cookie",
            serialize_1(new_cookie.name, new_cookie.value, new_cookie.options)
          );
        }
        return response;
      }
...
var Server = class {
  ...
  async init({ env }) {
    ...
    if (!this.options.hooks) {
      const module = await import("./hooks.server-HV3C2M67.js");
